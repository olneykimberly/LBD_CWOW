...
all.equal(colnames(counts), (metadata$RNA.config.ID))
```
# Create DGE object
```{r DGE_object}
# create object
...
# Save R object
```{r, eval=FALSE}
saveRDS(dge, file = paste0("../../rObjects/LBD_SCC_dge.raw.rds"))
dge.filtered.norm <- readRDS(paste0("../../rObjects/LBD_SCC_dge.raw.rds"))
```
...
group_colors <- c(control_color, 
                  control_AD_color, 
                  control_PA_color, 
                  LBD_color)[dge.tech$samples$TYPE]
...
colnames(design) <- c(control,control_AD, control_PA, condition, "sex", "RIN")
```
...
pcaWithMetadata$group <- factor(pcaWithMetadata$TYPE,
                                 levels = c(control, control_AD, control_PA, LBD, NA))
# Plotting
...
#  xlim(-150, 150) +
#  ylim(-150, 150) +
  scale_color_manual(values = c(control_color, control_AD_color,
                                control_PA_color, LBD_color,
                                "azure3")) +
  scale_shape_manual(values = c(control_shape, control_AD_shape,
                                control_PA_shape, LBD_shape)) +
  theme(
...
contrasts <- makeContrasts(
  ...
  LBDvsControl_AD = LBD - CONTROL_AD,
  LBDvsControl_PA = LBD - CONTROL_PA,
  Control_ADvsControl = CONTROL_AD - CONTROL, 
  Control_PAvsControl = CONTROL_PA - CONTROL,
  Control_ADvsControl_PA = CONTROL_AD - CONTROL_PA, 
