...
```{r zscore_byType}
# get cell types
Zscore_CellType_melt$Var1 <- as.factor(Zscore_CellType_melt$Var1)
# save levels to loop through to make a dataframe for each
CellTypes <- levels(Zscore_CellType_melt$Var1)

for (i in CellTypes) {
  CellType_df <-
    Zscore_CellType_melt[Zscore_CellType_melt$Var1 == i,]
...
plotnames = imap(violin_plots, ~paste0("../../results/star/cell_type_enrichment/umbrella_cell_type_BrainInABlender/TYPE.", .y, ".pdf")) %>%
  flatten()
plotnames
...
```
# NOTE!! 
Now go back to script 04a to create a new dge object that includes these cell type enrichment values. 
Will need to edit the file_paths_and_colours to include the updated metadata that has the cell type enrichment values. 
