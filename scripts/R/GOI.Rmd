...
voomCountsMatrix <- readRDS("/research/labs/neurology/fryer/m239830/LBD_CWOW/rObjects/ATS.voomCountsMatrix.rds")
```

```{r}
# get the gene id that corresponds to the gene name 
genes <- dge.filtered.norm$genes # gene information 
goi <- subset(genes, gene_name == "HPGD") # gene of interest 
# subset counts 
goi_counts <- subset(voomCountsMatrix, rownames(voomCountsMatrix) %in% goi)
...
```{r}
column_variables <- c("ENSG00000164120.14") # gene_id to plot expression data for
df$VaD <- as.factor(df$VaD) # make sure that the variable to compare between is a factor 
my_comparisons <- list(c("0", "1")) # define comparisons for pairwise stats
#colors <- brewer.pal(n = 5, name = "Set2") # colors red to blue 
colors <- TypeColors
```
...
plotnames = imap(violin_plots, ~paste0("../../results/", tool, "/goi/HPGD.TYPE.pdf")) %>%
  flatten()
plotnames
