...
shared_up_enric_heatmap <- 
  ...
  scale_fill_gradientn(
    ...
    limits = c(-25,-1),
          ...
          plot.title = element_text(size = 9, hjust = -2.25, vjust=0, margin = margin(0,0,0,0)))
...
```{r}
down_enrich_plot <-
  ggplot(data = down_enrich_results_subset, aes(x = InTerm, y = rev(Description))) +
...
up_enrich_plot <-
  ggplot(data = up_enrich_results_subset, aes(x = InTerm, y = rev(Description))) +
  ggplot2::facet_grid(~ Cluster) +
  geom_bar(stat = "identity", aes(fill = LogP), width = .65) +
  theme_bw() +
  scale_y_discrete(expand = c(0, 0)) +
...
jpeg(file="../../../results/manuscript_figures/Figure_2_AD_vs_LBD.jpeg",
