---
title: "Supplemental AD GO term gene heatmap"
...
path <- paste0("../../../results/manuscript_figures/Supplemental_AD_vs_Control_metascape")
...
up_input <- list(
  ...
  "Cellular responses to stress" = up_gene_char[[2]],
  "cellular response to cytokine stimulus" = up_gene_char[[3]],
  "TYROBP causal network in microglia" = up_gene_char[[4]],
  "actin filament-based process" = up_gene_char[[5]],
  "in utero embryonic development" = up_gene_char[[6]],
  "positive regulation of cytokine production" = up_gene_char[[7]],
  "negative regulation of cell differentiation" = up_gene_char[[8]],
  "positive regulation of cell death" = up_gene_char[[9]],
  "Viral Infection Pathways" = up_gene_char[[10]],
  "hemopoiesis" = up_gene_char[[11]],
  "Cellular response to starvation" = up_gene_char[[12]],
  "apoptotic signaling pathway" = up_gene_char[[13]],
  "positive regulation of protein localization" = up_gene_char[[14]],
  "positive regulation of immune response" = up_gene_char[[15]],
  "tube morphogenesis" = up_gene_char[[16]],
  "IL-18 signaling pathway" = up_gene_char[[17]],
  "regulation of cytoskeleton organization" = up_gene_char[[18]],
  "regulation of kinase activity" = up_gene_char[[19]],
  "protein phosphorylation" = up_gene_char[[20]]
)
...
# write table 
write.table(format_order_up_df, "Supplemental_AD_vs_Control_Metascape_upregulated_genes_within_GO.txt", sep= "\t", quote = FALSE, row.names = FALSE)
```
# down
```{r}
down_enrich_df <-
...
down_input <- list(
  ...
  "Pyruvate metabolism and Citric Acid (TCA) cycle" = down_gene_char[[2]],
  "nucleobase-containing small molecule metabolic process" = down_gene_char[[3]],
  "Valine, leucine and isoleucine degradation" = down_gene_char[[4]],
  "dicarboxylic acid metabolic process" = down_gene_char[[5]],
  "monocarboxylic acid metabolic process"  = down_gene_char[[6]],
  "behavior" = down_gene_char[[7]],
  "synaptic signaling"   = down_gene_char[[8]],
  "modulation of chemical synaptic transmission"  = down_gene_char[[9]],
  "cellular component morphogenesis"  = down_gene_char[[10]],
  "small molecule biosynthetic process"  = down_gene_char[[11]],
  "cilium assembly" = down_gene_char[[12]],
  "regulation of secretion" = down_gene_char[[13]],
  "inorganic ion transmembrane transport"= down_gene_char[[14]],
  "amide metabolic process"= down_gene_char[[15]],
  "brain development"  = down_gene_char[[16]],
  "Fatty acid metabolism" = down_gene_char[[17]],
  "response to salt" = down_gene_char[[18]],
  "microtubule-based transport"= down_gene_char[[19]],
  "Amino acid metabolism" = down_gene_char[[20]]
)
...
# write table 
write.table(format_order_down_df, "Supplemental_AD_vs_Control_Metascape_downregulated_genes_within_GO.txt", sep= "\t", quote = FALSE, row.names = FALSE)
```
# Up heatmap
...
GO_4_unique <- GO_4[!duplicated(GO_4[,c('gene')]),]
GO_4_top <- head(GO_4_unique, 40)

# lock in gene order which is sorted by adjusted p-value
GO_4_DEG <- subset(DEG_df, DEG_df$gene_id %in% GO_4_top$gene_id)
GO_4_DEG$FC <- 2^(GO_4_DEG$logFC)
# heatmap 
up_GO_4_heat <- ggplot(data = GO_4_DEG, aes(x = gene_type, y = reorder(gene, FC))) +
  geom_tile(aes(fill = FC), color = "gray") +
  geom_text(aes(label = sprintf("%0.2f", round(FC, digits = 2))), color = "black", size = 2.75) +
  scale_fill_gradientn(colours = c("blue","#FFFFCC","red"), 
                         values = rescale(c(-2.8,0,2.1)),
...
GO_down_3 <-
  subset(
    down_df,
    (
 down_df$variable == "Fatty acid metabolism" 
    )
    & down_df$value == 1
  )
...
path <- paste0("../../../results/manuscript_figures/Supplemental_Figure_AD_vs_Control_metascape_gene_FC_heatmaps")
