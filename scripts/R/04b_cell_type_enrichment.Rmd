...
BinB_voom <- Sir_UnMixALot(userInput=voomCounts_gene_name, dataColumns=c(2:620), geneColumn=631, species="human")
setwd(".")
```
reformat
```{r}
...
for (i in CellTypes) {
  path <- paste0("../../results/",tool,"/cell_type_enrichment/BrainInABlender/", condition,i,".txt") 
  assign(paste0(i),
         read.delim(
           path,
           header = TRUE,
...
violin_plot_fun <- function(i, j) {
  ggplot(cellType, aes(TYPE, i,  fill = sex_inferred)) +
   ...
   # geom_jitter(shape=16, position=position_dodge(1.2),  aes(fill=sex_inferred)) +
    theme_bw() + 
    ggtitle(j) +
    xlab("Disease group") +
    ylab(j) +
...
plotnames = imap(violin_plots, ~paste0("../../results/star/cell_type_enrichment/BrainInABlender/ByTypeEachSex.", .y, ".pdf")) %>%
  flatten()
walk2(plotnames, violin_plots, ~ggsave(filename = .x, plot = .y, 
                                    height = 4, width = 5.5))
...
plotnames = imap(violin_plots, ~paste0("../../results/star/cell_type_enrichment/BrainInABlender/ByTypeFacetSex.", .y, ".pdf")) %>%
  flatten()
walk2(plotnames, violin_plots, ~ggsave(filename = .x, plot = .y, 
                                    height = 4, width = 8.5))
```
...
```{r}
cellType$ATS_names <- factor(cellType$ATS_names, levels = c("no pathology", "amyloid", "amyloid + tau", "pure synuclein", "low amyloid + synuclein", "high amyloid + synuclein", "amyloid + synuclein + tau"))

metadata_continuous <-
  data.frame(cellType$Astrocyte.Zscore, cellType$Endothelial.Zscore, cellType$Microglia.Zscore, cellType$Mural.Zscore, cellType$Neuron_All.Zscore, cellType$Neuron_Interneuron.Zscore, cellType$Neuron_Projection.Zscore, cellType$Oligodendrocyte.Zscore, cellType$Oligodendrocyte_Immature.Zscore, cellType$RBC.Zscore)
column_variables <- c("Astrocyte.Zscore", "Endothelial.Zscore", "Microglia.Zscore", "Mural.Zscore", "Neuron_All.Zscore", "Neuron_Interneuron.Zscore", "Neuron_Projection.Zscore", "Oligodendrocyte.Zscore", "Oligodendrocyte_Immature.Zscore", "RBC.Zscore")

my_comparisons <- list(c("no pathology", "amyloid"), c("no pathology", "amyloid + tau"), c("no pathology", "low amyloid + synuclein"), 
                       c("no pathology", "high amyloid + synuclein"), c("no pathology","amyloid + synuclein + tau"))
...
plotnames = imap(violin_plots, ~paste0("../../results/star/cell_type_enrichment/BrainInABlender/ByATS.", .y, ".pdf")) %>%
  flatten()
plotnames
walk2(plotnames, violin_plots, ~ggsave(filename = .x, plot = .y, 
                                    height = 6, width = 6.5))
...
plotnames = imap(violin_plots, ~paste0("../../results/star/cell_type_enrichment/BrainInABlender/ByATSFacetSex.", .y, ".pdf")) %>%
  flatten()
plotnames
walk2(plotnames, violin_plots, ~ggsave(filename = .x, plot = .y, 
                                    height = 6, width = 11))
...
kruskal.test(Astrocyte.Zscore ~ ATS_names, data = cellType)
kruskal.test(Endothelial.Zscore ~ ATS_names, data = cellType)
kruskal.test(Microglia.Zscore ~ ATS_names, data = cellType)
kruskal.test(Mural.Zscore ~ ATS_names, data = cellType)
kruskal.test(Neuron_All.Zscore ~ ATS_names, data = cellType)
kruskal.test(Neuron_Interneuron.Zscore ~ ATS_names, data = cellType)
kruskal.test(Neuron_Projection.Zscore ~ ATS_names, data = cellType)
kruskal.test(Oligodendrocyte.Zscore ~ ATS_names, data = cellType)
kruskal.test(Oligodendrocyte_Immature.Zscore ~ ATS_names, data = cellType)
kruskal.test(RBC.Zscore ~ ATS_names, data = cellType)

wilcox.test(Astrocyte.Zscore ~ sex_inferred, data = cellType)
wilcox.test(Endothelial.Zscore ~ sex_inferred, data = cellType)
wilcox.test(Microglia.Zscore ~ sex_inferred, data = cellType)
wilcox.test(Mural.Zscore ~ sex_inferred, data = cellType)
wilcox.test(Neuron_All.Zscore ~ sex_inferred, data = cellType)
wilcox.test(Neuron_Interneuron.Zscore ~ sex_inferred, data = cellType)
wilcox.test(Neuron_Projection.Zscore ~ sex_inferred, data = cellType)
wilcox.test(Oligodendrocyte.Zscore ~ sex_inferred, data = cellType)
wilcox.test(Oligodendrocyte_Immature.Zscore ~ sex_inferred, data = cellType)
wilcox.test(RBC.Zscore ~ sex_inferred, data = cellType)


pairwise.wilcox.test(cellType$Microglia.Zscore, cellType$ATS_names,
                 p.adjust.method = "BH")

pairwise.wilcox.test(cellType$Neuron_Projection.Zscore, cellType$ATS_names,
                 p.adjust.method = "BH")
```
