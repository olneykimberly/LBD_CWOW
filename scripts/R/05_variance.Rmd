...
path <- paste0("../../results/", tool ,"/varpart/", condition, "_gene_CCA_all_varaiables")
saveToPDF(paste0(path, ".pdf"), width = 16, height = 16)
```
### variables that most samples have information for
variables that most samples have information for. 
...
path <- paste0("../../results/", tool ,"/varpart/", condition, "_gene_CCA_most_samples_have_variable_information")
...
path <- paste0("../../results/", tool, "/varpart/", condition, "_varpart_type_sex_race_apoe_age_rin")
...
write.table(variance_explained, 
            paste0("../../results/", tool ,
            "/varpart/", condition, "_variance_explained.tsv"), 
            sep = "\t", 
            quote = FALSE)
```
...
path <- paste0("../../results/", tool ,"/varpart/", condition, "_CCA_type_sex_race_apoe_age_rin")
...
Voom - Transform count data to log2-counts per million (logCPM), estimate the mean-variance relationship and use this to compute appropriate observational-level weights. The data are then ready for linear modeling.
```{r}
# subtract out effect of: RIN 
modelFit <- fitVarPartModel(v, ~RIN, dge.filtered.norm$samples, fxn=residuals)
residMatrix = do.call(rbind, modelFit)
```
...
formula <- ~ (1|TYPE) +
  ...
  (1|APOE) + 
  Age 
varPartResid <- fitExtractVarPartModel(residMatrix, formula, dge.filtered.norm$samples)
```

```{r}
plotVarPart(sortCols(varPartResid), label.angle=60)

path <- paste0("../../results/", tool, "/varpart/", condition, "_varpart_type_sex_race_apoe_age_regressed_rin")
saveToPDF(paste0(path, ".pdf"), width = 8.2, height = 5.2)

varPartResid$gene_id <- rownames(varPartResid)
variance_explained <- merge(varPartResid, genes, by = "gene_id")
write.table(variance_explained, 
            paste0("../../results/", tool ,
            "/varpart/", condition, "_variance_explained_regressed_RIN.tsv"), 
            sep = "\t", 
            quote = FALSE)
```
...
path <- paste0("../../results/", tool ,"/varpart/", condition, "_gene_CCA_all_varaiables_PC1_4")
saveToPDF(paste0(path, ".pdf"), width = 16, height = 16)
```
### variables in model
```{r}
...
path <- paste0("../../results/", tool ,"/varpart/", condition, "_gene_CCA_type_rin_sex_apoe_age_PC1_4")
saveToPDF(paste0(path, ".pdf"), width = 6, height = 6)
```

```{r}
saveRDS(veBayesFit, file = paste0("../../rObjects/", condition, "_", min_expression,".veBayesFit.rds"))
saveRDS(voomCountsMatrix, file = paste0("../../rObjects/", condition, "_", min_expression,".voomCountsMatrix.rds"))

veBayesFit <- readRDS(paste0("../../rObjects/", condition, "_", min_expression,".veBayesFit.rds"))
voomCountsMatrix <- readRDS(paste0("../../rObjects/", condition, "_", min_expression,".voomCountsMatrix.rds"))
```
