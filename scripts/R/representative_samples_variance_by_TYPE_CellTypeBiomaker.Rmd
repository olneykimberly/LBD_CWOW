...
condition <- c("representative_samples") #  TYPE
...
all.equal(dge.filtered.norm$samples$NPID, (info$NPID))
```
...
df <- df[ order(match(df$NPID, info$NPID)), ]
scaled.info <-
  df[c(
    "RIN",
    "Age",
...
```{r}
setnames(varPart, old = c('flowcell_and_lane','sex_inferred'), 
         new = c('Batch','Sex'))
...
for (i in allComparisons) {
  ...
  path <- paste0("../../results/", tool, "/DEGs/", condition, "_", 
  i, "_gene_DEGs_FDRq1_logFC_1.txt", sep = "") 
  write.table(
    vTopTable1,
    path,
    sep = "\t",
...
# Plotting
ggplot(data = pcaWithMetadata, aes(x = PC1, y = PC2, shape = group, 
                                   color = sex_inferred)) +
  geom_point(size = 2.5) +
  theme_bw() +
