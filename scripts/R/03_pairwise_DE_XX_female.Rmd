...
### metadata
```{r read_data}
# subset metadata file to only include samples of interest
metadata <- subset(metadata, sex_inferred == "female")
...
TypeColors <- c(TypeColors)[dge$samples$TYPE]
```
...
path <- paste0("../../results/", tool, "/MDS/", 
               condition,"_MDS_label_TYPE_dim1&2")
...
plotMDS(
  lcpm,
  top = 100, 
  labels = dge.filtered.norm$samples$TYPE,
  cex = .8, 
  dim.plot = c(1,2), 
  plot = TRUE, 
  col = TypeColors,
...
path <- paste0("../../results/", tool, "/MDS/", 
               condition,"_gene_MDS_label_TYPE_dim1&2")
saveToPDF(paste0(path, ".pdf"), width = 5.2, height = 5.2)
```
...
design <- model.matrix(~ 0 + TYPE + RIN + Age + APOE + Race,  dge.filtered.norm$samples)
colnames(design) <- c(CONTROL, AD, PA, LBD, "RIN", "Age", "E2E4", "E3E3", "E3E4", "E4E4", "Eastern_Indian", "Hispanic")
```
...
vTopTableAll <-
  subset(
    ...
    saveRDS(vTopTableAll, file = 
            paste0("../../rObjects/", 
                   i, "_", condition,"_gene_table.rds"))
  path <- paste0("../../results/", tool, "/DEGs/",
  i, "_", condition, "_gene_DEGs_FDRq1.00.txt", sep = "") 
  write.table(
    vTopTableAll,
    path,
    sep = "\t",
  ...
  path <- paste0("../../results/", tool, "/DEGs/",
  i, "_", condition, "_gene_DEGs_FDRq0.05.txt", sep = "") 
  write.table(
    vTopTable1,
    path,
    sep = "\t",
...
p_vol <-
  ...
  geom_text_repel(data = up10,
                  ...
                  fontface="italic",
                  size = 4, 
                  max.overlaps = getOption("ggrepel.max.overlaps", default = 14)
                  ) +
...
# save
path <- paste0("../../results/", tool, "/volcano/PA_", condition, "_gene_volcano_FDRq0.05")
saveToPDF(paste0(path, ".pdf"), width = 4, height = 4.25)
```
# glimma
```{r}
