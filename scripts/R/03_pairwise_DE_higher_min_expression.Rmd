...
help(filterByExpr)
tool <- "star_min_count_50"
keep.expr <- filterByExpr(dge, group = dge$samples$TYPE, min.count = 80)
dim(dge)
...
# Save R object
```{r}
saveRDS(dge.filtered.norm, file = paste0("../../rObjects/", tool, condition, ".dge.filtered.norm.rds"))
dge.filtered.norm <- readRDS(paste0("../../rObjects/", tool, condition ,".dge.filtered.norm.rds"))
```
