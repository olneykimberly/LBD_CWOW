...
gene_info <- merge(genes, biomart_gene_lengths, by = "gene_id")
dge.filtered.norm$genes$gene_id <- gsub("\\..*","",dge.filtered.norm$genes$gene_id)
gene_info <- merge(dge.filtered.norm$genes, biomart_gene_lengths, by = "gene_id", all.x=T)

gene_info <- gene_info[match(dge.filtered.norm$genes$gene_id, gene_info$gene_id),] 
rpkm_protein_coding <- rpkm(dge.filtered.norm, gene.length = gene_info$length)
...
write.table(
  rpkm_protein_coding,
  paste0("../../results/", tool, "/counts/", condition, "_", min_expression, "_rpkm.txt"),
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)
NO_DUPS <- rpkm_protein_coding[!duplicated(rownames(rpkm_protein_coding)), ]
...
xCell_scores <- xCellAnalysis(NO_DUPS)
library(tidyverse)
...
colnames(xCell_scores) <- dge.filtered.norm$samples$sex_inferred
heatmap(
