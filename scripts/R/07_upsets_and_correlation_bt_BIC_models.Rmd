...
condition <- c("protein_coding_cellType_zscores")

allComparisons <- c("BIC_LBDvsControl", 
                    "BIC_LBDvsAD", 
                    "BIC_LBDvsPA", 
                    "BIC_ADvsControl", 
                    "BIC_PAvsControl", 
...
allComparisons <- c("short_model_BIC_LBDvsControl", 
                    "short_model_BIC_LBDvsAD", 
                    "short_model_BIC_LBDvsPA", 
                    "short_model_BIC_ADvsControl", 
                    "short_model_BIC_PAvsControl", 
                    "short_model_BIC_ADvsPA")
allComparisons 
...
ct_LBDvsControl_up <- subset(BIC_LBDvsControl, logFC > 0.25 & adj.P.Val <= 0.05)
ct_LBDvsControl_down <- subset(BIC_LBDvsControl, logFC < -0.25 & adj.P.Val <= 0.05)
ct_ADvsControl_up <- subset(BIC_ADvsControl, logFC > 0.25 & adj.P.Val <= 0.05)
ct_ADvsControl_down <- subset(BIC_ADvsControl, logFC < -0.25 & adj.P.Val <= 0.05)
ct_PAvsControl_up <- subset(BIC_PAvsControl, logFC > 0.25 & adj.P.Val <= 0.05)
ct_PAvsControl_down <- subset(BIC_PAvsControl, logFC < -0.25 & adj.P.Val <= 0.05)

# ct stands for cell type short model 
cts_LBDvsControl_up <- subset(short_model_BIC_LBDvsControl, logFC > 0.25 & adj.P.Val <= 0.05)
cts_LBDvsControl_down <- subset(short_model_BIC_LBDvsControl, logFC < -0.25 & adj.P.Val <= 0.05)
cts_ADvsControl_up <- subset(short_model_BIC_ADvsControl, logFC > 0.25 & adj.P.Val <= 0.05)
cts_ADvsControl_down <- subset(short_model_BIC_ADvsControl, logFC < -0.25 & adj.P.Val <= 0.05)
cts_PAvsControl_up <- subset(short_model_BIC_PAvsControl, logFC > 0.25 & adj.P.Val <= 0.05)
cts_PAvsControl_down <- subset(short_model_BIC_PAvsControl, logFC < -0.25 & adj.P.Val <= 0.05)
```

# Upset of DEGs
```{r}
list_input <- list("AD down-regulated" = ADvsControl_down$gene_name,
                   "AD up-regulated" = ADvsControl_up$gene_name,
                   "ct AD down-regulated" = ct_ADvsControl_down$gene_name,
                   "ct AD up-regulated" = ct_ADvsControl_up$gene_name,
                   "cts AD down-regulated" = cts_ADvsControl_down$gene_name,
                   "cts AD up-regulated" = cts_ADvsControl_up$gene_name
                   )
data <- fromList(list_input)
subset(data, rownames(data) == "SNCA")
```
### Upset between LBDvsControl with and without correcting for cell type 
```{r}
list_input <- list("LBD down-regulated" = LBDvsControl_down$gene_name,
                   "LBD up-regulated" = LBDvsControl_up$gene_name,
                   "ct LBD down-regulated" = ct_LBDvsControl_down$gene_name,
                   "ct LBD up-regulated" = ct_LBDvsControl_up$gene_name,
                   "cts LBD down-regulated" = cts_LBDvsControl_down$gene_name,
                   "cts LBD up-regulated" = cts_LBDvsControl_up$gene_name
                   )
data <- fromList(list_input)
subset(data, rownames(data) == "TMEM175")
upset(data, intersect = c('LBD down-regulated',
        'ct LBD down-regulated',
        'cts LBD down-regulated',
        'LBD up-regulated',
        'ct LBD up-regulated', 
        'cts LBD up-regulated'))
```
...
upset_gene <- upset(data, set_sizes = FALSE,
  ...
  queries=list(
    ...
    upset_query(set='ct LBD down-regulated', fill='blue'),
    upset_query(set='cts LBD down-regulated', fill='blue'),   
    upset_query(set='LBD up-regulated', fill='red'),
    upset_query(set='ct LBD up-regulated', fill='red'),
    upset_query(set='cts LBD up-regulated', fill='red')
  ),
  intersections = list(c('LBD down-regulated', 'ct LBD down-regulated', 'cts LBD down-regulated'),
                      c('LBD down-regulated', 'ct LBD down-regulated'), 
                       'LBD down-regulated',
                       'ct LBD down-regulated', 
                       c('LBD up-regulated', 'ct LBD up-regulated'), 
                       'LBD up-regulated',
                       'ct LBD up-regulated', 
                       c('LBD up-regulated', 'ct LBD down-regulated'),
                       c('ct LBD up-regulated', 'LBD down-regulated')
...
path <- paste0("../../results/", tool, 
               "/upset/", condition, "_LBD_vs_LBD_FDRq0.05_logFC_0.25")
...
path <- paste0("../../results/", tool,
               "/upset/", condition, "_LBD_vs_LBD_FDRq0.05_logFC_0.25_simple")
...
```{r}
LBDandLBD <-
  merge(BIC_LBDvsControl,
        LBDvsControl,
        by = "gene_name",
        all = T)
LBDandLBD <- LBDandLBD[c(1,2,6,7,8:15,22:29)]
colnames(LBDandLBD) = c("gene_name", "chr", "gene_id","gene_type", "AD_logFC", "AD_CI.L", "AD_CI.R", "AD_AveExpr", "AD_t","AD_P.val", "AD_adj.P.Val", "AD_B", "LBD_logFC", "LBD_CI.L", "LBD_CI.R", "LBD_AveExpr", "LBD_t","LBD_P.val", "LBD_adj.P.Val", "LBD_B")

LBDandLBD_goi <- subset(LBDandLBD, 
...
ADandLBD_corr_plot <- ggplot(data = LBDandLBD, 
