...
### Dot plot markers
```{r}
markers.to.plot <- c("SLC1A2", "GJA1", "CLDN5", "TGM2", "STAB1", "CD74", "VCAN", "TNR", "S100B", "ABCA2", "NRG1", "MEG3", "ERBB4", "GAD1")
dot <- DotPlot(dataObject.annotated, features = markers.to.plot, cols = sample_colors, dot.scale = 8, split.by = "sample") +
    RotatedAxis()
```
...
### Save RDS 
```{r save_annotations}
saveRDS(dataObject.annotated, paste0("../../rObjects/",tolower(tissue),"_annotated_harmony.rds"))
```
